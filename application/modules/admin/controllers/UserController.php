...
class Admin_UserController extends Base_Model_ObtorLib_App_Admin_Controller {
    ...
    public function viewLogAction() {
        try {
            ...
            $userType = 'admin';
             ...
             // get all the user all users...
            $systemLogService = new Base_Model_ObtorLib_App_Core_System_Service_Log();
            $systemLogEntity = new Base_Model_ObtorLib_App_Core_System_Entity_Log();
            $systemLogEntity->setUserId($userId);
            $systemLogEntity->setUserType($userType);
            $systemLogService->log = $systemLogEntity;
            
            $objPaggination = new Base_Model_ObtorLib_Base_ObtorLib_Paggination();
    ...
    public function historyAction() {
        try {
            $pageHeading = "User-History";
            $this->view->page_heading = $pageHeading;
            $this->view->data_page = "forms";

            // get all the country...
            $userHistoryService = new Base_Model_ObtorLib_App_Core_User_Service_UserLoginHistory();
            $userHistoryEntity = new Base_Model_ObtorLib_App_Core_User_Entity_UserLoginHistory();
            
            $userHistoryService->userLoginHistory = $userHistoryEntity;
            
            $objPaggination = new Base_Model_ObtorLib_Base_Lib_Paggination();
            $page = $this->_getParam('page', 1);
            $objPaggination->CurrentPage = $page;
            $objPaggination->TotalResults = $userHistoryService->searchCount();
            $paginationData = $objPaggination->getPaggingData();
            $pageLimit1 = $paginationData['MYSQL_LIMIT1'];
            $pageLimit2 = $paginationData['MYSQL_LIMIT2'];
            ...
            $userHistoryService->userLoginHistory = $userHistoryEntity;
            $userHistoryInfo = $userHistoryService->search($limit);
            $this->view->userHistoryInfo = $userHistoryInfo;
